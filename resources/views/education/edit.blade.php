...
@section('content')
    <div class="p-20 flex-col  justify-center container text-white">
        ...
        <div class="text-left  w-9/12 mr-auto ">
            <h2 class="font-bold">Edit Education: </h2>
        </div>

        <form class="flex-col w-11/12 ml-auto mr-auto justify-center" action="{{ route('education.update', $education) }}"
            method='POST'>
            @csrf
            @method('PUT')
            ...
            <label for="degree" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Degree:
                <input type="text" placeholder="degree" name="degree" id="degree" value="{{ $education->degree }}"
                    class="bg-gray-100  text-gray-900 text-sm rounded block w-1/2 p-2.5">
            </label>

            <label for="field_of_study" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                Field of Study:
                <input type="text" placeholder="faculty" name="field_of_study" id="field_of_study"
